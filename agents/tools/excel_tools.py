"""
Tools for processing Excel files and extracting data for the database.
"""
import os
import pandas as pd
...
class ReadExcelInput(BaseModel):
    ...
    sheet_name: str = Field("Sheet1", description="Name of the sheet to read")


class ReadExcelTool(BaseTool):
    name = "read_excel"
    description = "Read data from an Excel file"
    args_schema = ReadExcelInput

    def _run(self, file_path: str, sheet_name: str = "Sheet1") -> Dict:
        """Read data from an Excel file and return as a dictionary."""
...
class ImportUsersFromExcelInput(BaseModel):
    file_path: str = Field(..., description="Path to the Excel file")
    sheet_name: str = Field("Sheet1", description="Name of the sheet to read")
    username_col: str = Field(..., description="Column name for username")
    email_col: Optional[str] = Field(None, description="Column name for email")
    full_name_col: Optional[str] = Field(None, description="Column name for full name")
    profile_data_cols: Optional[List[str]] = Field(None, description="Columns to include in profile data")


class ImportUsersFromExcelTool(BaseTool):
    name = "import_users_from_excel"
    description = "Import users from an Excel file into the database"
    args_schema = ImportUsersFromExcelInput

    def _run(self, file_path: str, sheet_name: str = "Sheet1", username_col: str = None,
...
class ImportGroupsFromExcelInput(BaseModel):
    file_path: str = Field(..., description="Path to the Excel file")
    sheet_name: str = Field("Sheet1", description="Name of the sheet to read")
    name_col: str = Field(..., description="Column name for group name")
    description_col: Optional[str] = Field(None, description="Column name for group description")


class ImportGroupsFromExcelTool(BaseTool):
    name = "import_groups_from_excel"
    description = "Import groups from an Excel file into the database"
    args_schema = ImportGroupsFromExcelInput

    def _run(self, file_path: str, sheet_name: str = "Sheet1", name_col: str = None,
...
class ImportUserGroupMappingsFromExcelInput(BaseModel):
    file_path: str = Field(..., description="Path to the Excel file")
    sheet_name: str = Field("Sheet1", description="Name of the sheet to read")
    username_col: str = Field(..., description="Column name for username")
    group_name_col: str = Field(..., description="Column name for group name")
    role_col: Optional[str] = Field(None, description="Column name for role")


class ImportUserGroupMappingsFromExcelTool(BaseTool):
    name = "import_user_group_mappings_from_excel"
    description = "Import user-group mappings from an Excel file into the database"
    args_schema = ImportUserGroupMappingsFromExcelInput
...
class ImportActivitiesFromExcelInput(BaseModel):
    file_path: str = Field(..., description="Path to the Excel file")
    sheet_name: str = Field("Sheet1", description="Name of the sheet to read")
    username_col: str = Field(..., description="Column name for username")
    activity_type_col: str = Field(..., description="Column name for activity type")
    description_col: str = Field(..., description="Column name for description")
    points_col: Optional[str] = Field(None, description="Column name for points")
    group_name_col: Optional[str] = Field(None, description="Column name for group name")
    timestamp_col: Optional[str] = Field(None, description="Column name for timestamp")
    metadata_cols: Optional[List[str]] = Field(None, description="Columns to include in metadata")


class ImportActivitiesFromExcelTool(BaseTool):
    name = "import_activities_from_excel"
    description = "Import activities from an Excel file into the database"
    args_schema = ImportActivitiesFromExcelInput
...
EXCEL_TOOLS = [
    ReadExcelTool(),
    ImportUsersFromExcelTool(),
    ImportGroupsFromExcelTool(),
    ImportUserGroupMappingsFromExcelTool(),
    ImportActivitiesFromExcelTool()
]
